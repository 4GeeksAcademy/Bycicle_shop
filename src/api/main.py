...
def product_post():
    ...
    if cart:
        new_cart_item = ShoppingCartItem(
            cart_id=cart.id, bicycle_id=bicycle_id, quantity=quantity
        )
        ...
        print(new_cart)
        new_cart_item = ShoppingCartItem(
            cart_id=new_cart.id, bicycle_id=bicycle_id, quantity=quantity
        )
        # add the new user to the database
        db.session.add(new_cart_item)
        db.session.commit()
...
def user_carts():
    ...
    if cart:
        cartItems = ShoppingCartItem.query.filter_by(
            cart_id=cart.id
        ...
        response = {"success": "true", "shooping_cart": {}, "shooping_cart_item": {}}
        return response
